...
public class ProximityGraph implements IGraphAssembler {
	...
	public void buildGraph(Vector<Element> boxes, Vector<TextBox> textos,
		...
		for (int i = 0; i < caminos.size(); i++) {
			...
			if (validConnection(b1andb2[0], b1andb2[1], boxes)) {
				p.connects(b1andb2[0], b1andb2[1]);// El path guarda que box
													// une, es informacion util
													// despues!
				caminosValidos.add(p);

				Rect r1 = Imgproc.boundingRect(boxes.get(b1andb2[0])
						.getPoints());
				Core.rectangle(graphBuild, r1.tl(), r1.br(), new Scalar(255));// //////
				Rect r2 = Imgproc.boundingRect(boxes.get(b1andb2[1])
						.getPoints());
		...
		for (int i = 0; i < textos.size(); i++) {
			// Unir cada texto a la caja que lo contiene o a la flecha mas
			// cercana
	...
	private void findClosestBox(Point ext1, Point ext2, int[] b1andb2,
		...
		for (int l = 0; l < boxes.size(); l++) {
			...
			// Minima distancia que existe entre un punto extremo y un set de
			// puntos de la caja
	...
	private int minimumDistance(Point ext, MatOfPoint puntos) {
		...
		for (int i = 0; i < pts.size(); i++) {
			...
			Point p = pts.get(i);// new
									// Point(puntos.get(0,i)[0],puntos.get(0,i)[1]);
			long actual = (long) (p.x - ext.x) * (long) (p.x - ext.x)
					+ (long) (p.y - ext.y) * (long) (p.y - ext.y);
		...
		// System.out.println(Double.toString(Math.sqrt(res)) + " Vs " +
		// Integer.toString(MAXIMUM_DISTANCE));
	...
	private int minimumDistanceS(Point p, Vector<Vec4i> segments) {
		...
		for (int i = 0; i < segments.size(); i++) {
			...
			long actual = ((long) (p.x - p1.x)) * ((long) (p.x - p1.x))
					+ ((long) (p.y - p1.y)) * ((long) (p.y - p1.y));
			// sqrt(actual)//Para hacerlo mas rapido no calculo sqrt
			if (actual < res) {
				res = actual;
				devolver = i;
			}
			actual = (long) (p.x - p2.x) * (long) (p.x - p2.x)
					+ (long) (p.y - p2.y) * (long) (p.y - p2.y);
			// sqrt(actual)//Para hacerlo mas rapido no calculo sqrt
			if (actual < res) {
				res = actual;
				devolver = i;
			}
			actual = (long) (p.x - p3.x) * (long) (p.x - p3.x)
					+ (long) (p.y - p3.y) * (long) (p.y - p3.y);
			// sqrt(actual)//Para hacerlo mas rapido no calculo sqrt
			if (actual < res) {
				res = actual;
				devolver = i;
		...
		/*
		 * if (sqrt(res) < MAXIMUM_DISTANCE) return sqrt(res);//res else return
		 * -1;
		 */
	...
	private boolean validConnection(int b1, int b2, Vector<Element> boxes) {
		if ((b1 == -1) || (b2 == -1))// Alguna es -1, uno de los extremos no
										// llega a ninguna caja cerca
			return false;
		...
		if ((boxes.get(b1).getPadre() == b2)
				|| (boxes.get(b2).getPadre() == b1))// Una contiene a la otra?
			return false;
	...
	private void establecerTextoB(TextBox t, int box, Vector<Element> boxes) {
		...
		if (box == -1) {
			...
			// Core.rectangle(muestra,t.getArea().tl(),t.getArea().br(),new
			// Scalar(255));
			return;
		}
		muestra = Mat.zeros(s, CvType.CV_8UC1);
		Core.rectangle(muestra, t.getArea().tl(), t.getArea().br(), new Scalar(
				255));
	...
	private void establecerTextoP(TextBox t, int path,
		...
		if (path == -1) {
			// muestra=Mat.zeros(s,CvType.CV_8UC1);
			// Core.rectangle(muestra,t.getArea().br(),t.getArea().tl(),new
			// Scalar(255));
			return;
		}
	...
	private void drawPath(Mat m, Vector<Vec4i> path) {
		for (int i = 0; i < path.size(); i++) {
			Vec4i l = path.get(i);
			Core.line(m, new Point(l.v0, l.v1), new Point(l.v2, l.v3),
					new Scalar(255), 1, Core.LINE_AA, 0);
		}

	}

	private void text_findClosestBoxPathBuild(TextBox texto,
			Vector<Element> boxes, Vector<Relation> caminosValidos) {
		int d1 = Integer.MAX_VALUE, d2 = Integer.MAX_VALUE;
		int b1 = -1, p1 = -1;
		...
		for (int l = 0; l < boxes.size(); l++) {
			...
			// Minima distancia que existe entre un punto central y un set de
			// puntos de la caja
			int mD1 = minimumDistance(centro, b.getPoints());

			// Si hay un box mas cerca de lo que teniamos hasta ahora
			if ((mD1 != -1) && (d1 > mD1)
					&& (estaContenido(area, b.getPoints()))) {
				d1 = mD1;
				b1 = b.getId();
			}
		}
		...
		if ((b1 != -1) && (p1 != -1)) {
			...
			// Hay un path mas cerca
				...
				// Cuando el path se extendio un poco adentro de la caja, y el
				// texto esta contenido en alguno de los path que conecta
				/*
				 * if ( estaContenido(texto.getArea(),boxes[caminosValidos[p1].
				 * getConnection1()].getPoints()) )
				 * establecerTexto(texto,caminosValidos
				 * [p1].getConnection1(),caminosValidos); if (
				 * estaContenido(texto
				 * .getArea(),boxes[caminosValidos[p1].getConnection2
				 * ()].getPoints()) )
				 * establecerTexto(texto,caminosValidos[p1].getConnection2
				 * (),caminosValidos);
				 */
